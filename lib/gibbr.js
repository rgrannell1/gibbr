...
const parseArgs = function (args) {
	...
	if (!is.boolean(args.recur)) {
		throw TypeError("recur was not a boolean.")
	}
...
const rename = function (fpath, renamer, shouldRecur, shouldNuke) {
	...
	console.assert(is.string(fpath))
	console.assert(is.function(renamer))

	console.assert(is.boolean(shouldRecur))
	console.assert(is.boolean(shouldNuke))
...
const main = function (args) {
	...
	const renameFile = function (fpath) {
		rename(abspath(fpath), randomFileName, args.recur, args.nuke)
	}

	args.files[0] === '-' ?
		mapStandardInput(renameFile) :
		args.files.forEach(renameFile)
